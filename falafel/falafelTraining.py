...
def train(intentFile):
    """
    :param intentFile:
    :return:
    """
    lemmatizer = WordNetLemmatizer()
    intents = json.loads(open(f'{currentDir}/falafelDatabase/{intentFile}.json').read())
    # initlize three list
    words = list()
    documents = list()
    classes = list()
    ...
    pickle.dump(words, open(f'{currentDir}/falafelPickle/words{intentFile}.pkl', 'wb'))
    pickle.dump(classes, open(f'{currentDir}/falafelPickle/classes{intentFile}.pkl', 'wb'))
    ...
    model.save(f'{currentDir}/falafelModel/{intentFile}model.h5', hist)
    print(f'done for {intentFile}')
